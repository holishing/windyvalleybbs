...
/*cache.080724: 註冊帳號自動記錄在apply.log並每天轉到log板*/

void
applylog(mode, msg)		/* APPLY 一般記錄 */
  char *mode, *msg;
{
  char buf[512];

  sprintf(buf, "%s %s %-13s%s\n", Now(), mode, cuser.userid, msg);
  f_cat(FN_RUN_APPLY, buf);
}
  ...
  char *mode;
{
  ...
  if (!HAS_STATUS(STATUS_DATALOCK))	/* itoc.010811: 沒有被站長鎖定，才可以回存 .ACCT */
  {
    ...
    cuser.goodart = cutmp->goodart;
    cuser.badart = cutmp->badart;
...
acct_apply()
{
  ...
  /*080416.cache: 要同意遵守站規後才可以註冊*/
  if (more("gem/@/@rule", (char *) -1) >= 0)
    {
      if (vans("您是否同意以上的規定呢(Y/N)？[N] ") != 'y')
        {
          vmsg("不同意將無法註冊唷，請重新連線");
          login_abort("");
        }
    }
  ...
  do
  {
    if (vget(22, 0, "性別 (0)男性 (1)女性 [0]：", buf, 3, DOECHO))
      cuser.sex = buf[0] - '0';
  } while (cuser.sex < 0 || cuser.sex > 1);
  ...
  int mode;
{
  ...
  utmp.goodart = cuser.goodart;
  utmp.badart = cuser.badart;
  ...
  {
    char nick[9][5] = {"清流", "正咩", "訪客", "賭神", "希望", "好人", "阿宅",
    "型男", "系寶"};
    sprintf(cuser.username, "電機系的%s", nick[ap_start % 9]);
  }
#endif	/* GUEST_NICK */
...
#  ifdef GUEST_WHERE
  if (!cuser.userlevel)		/* guest */
  {
    ...
    char from[16][9] = {"系辦公室", "迅慧講堂", "系館中庭", "頂樓溫室",
    "防護網上", "垃圾桶中", "系館大門", "繁城講堂", "靄雲廳裡",
    "系Ｋ沙發", "奇美咖啡", "自強操場", "遺棄大樓", "自強草皮",
    "會辦倉庫", "系館後門"};
  ...
  char *content;
{
  ...
  move(b_lines, 0);
  outs("   ※ 無法連線時，請利用 port3456 上站  參觀帳號：\033[1;32m" STR_GUEST "\033[m 申請新帳號：\033[1;31m" STR_NEW "\033[m");
...
/*cache.080812: guest最多登入限制*/
      if (utmp_count(cuser.userno, 0) >= MAXGUEST)
        {
          vmsg("現在站上的guest帳號過多無法登入");
          login_abort("");
        }
...
tn_login()
{
  ...
  if (cuser.userlevel)		/* not guest */
  {
   ...
   prints("★ 歡迎您第 \033[1;33m%d\033[m 度隨風起舞，上次登入的時間是 \033[1;33m%s\033[m\n",
     cuser.numlogins, Btime(&cuser.lastlogin));
   prints("★ 來自於 \033[1;33m%s\033[m\n",
     cuser.lasthost);
...
/*
  ...
  {
    if(!i)
      vmsg("您的出生年月日不正確，請重新輸入");
    vget(22, 0, "出生年份 (民國)：", ans, 3, DOECHO);
  ...
  while (cuser.month < 1 || cuser.month > 12)
  {
    if(!i)
      vmsg("您的出生年月日不正確，請重新輸入");
    vget(22, 0, "出生月份 (01-12)：", ans, 3, DOECHO);
  ...
  while (cuser.day < 1 || cuser.day > 31)
  {
    if(!i)
      vmsg("您的出生年月日不正確，請重新輸入");
    vget(22, 0, "出生日期 (01-31)：", ans, 3, DOECHO);
  ...
  {
    vmsg("您的性別資料不正確，請重新輸入");
    cuser.sex = vget(22, 0, "性別 (0)男性 (1)女性：", ans, 2, DOECHO) - '0';
    j=1;
  }

  if (j)
    {
      vmsg("您有異動過個人資料，請立即重新上站");
      i=0;
      j=0;
    }
*/
...
//  move(b_lines -1, 0);
//  outs("請按任意鍵離開...");
//  if ( vkey() != 'b' || vkey() != 'u' || vkey() != 's' || vkey() != 'y' )
//    login_abort("");

/*cache.080714: 一般的關站(只顯示關站畫面)*/
//    login_close("");
...
/*cache.080510: --維修用,開站時要記得設定這段變成註解--*/
  ...
  time(&ap_start);
  getloadavg(load,3);
  prints("%s ⊙ " SCHOOLNAME " ⊙ " MYIPADDR CURRENTVER "\n"
    "歡迎光臨【\033[1;33;46m %s \033[m】目前線上人數 [%d] 人。系統負載：%.2f %.2f %.2f [%s]",
    str_host, str_site, ushm->count,load[0],load[1],load[2],load[0]>10?"\033[1;37;41m過高\033[m":load[0]>5?"\033[1;37;42m偏高\033[m":"\033[1;37;44m正常\033[m");
