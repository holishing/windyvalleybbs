  ...
  ACCT *acct;
{
  UTMP *up;
  int userno, rich;
  int hidden;/*cache.080814: 隱藏IP*/ 
  char *userid;
  char fortune[7][9] = {"沒什麼錢", "普普通通", "家境小康", "有不少錢", "家財萬貫", "財力雄厚", "富可敵國"};
  ...
  if(acct->gold >= 10000)
    ...
    rich=5;
  else if(acct->gold>=10 || acct->money >= 10000000)
    rich=4;    
  else if(acct->gold>=1 || acct->money >= 1000000)
    rich=3;
...
//    Btime(&acct->lastlogin), acct->lasthost);
    hidden ? "時間保密" : Btime(&acct->lastlogin),
    hidden ? "位址保密" : acct->lasthost);

  prints("[成就] 優文 %d 篇/劣文 %d 篇\n", acct->goodart, acct->badart);
