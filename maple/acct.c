...
/* cache.080805: 查找使用者資料show住址電話 */
/* 注意:sizeof(ACCT)可能會變大兩倍*/
  ...
  int adm;			/* 0: user info  1: admin  2: reg-form */
{
  ...
  /*080417.cache: 可能會有民國100年問題...*/ 
  prints("  \033[32m出生日期：\033[37m民國 %02d 年 %02d 月 %02d 日             \033[32m我的性別：\033[37m%.2s\n", u->year, u->month, u->day, u->sex ? "♀":"♂");
#ifdef BAD_POST
  prints("  \033[32m上站次數：\033[37m%-35d\033[32m文章篇數：\033[37m%d (優:%d/劣:%d)\n", u->numlogins, u->numposts, u->goodart, u->badart);
#else
  prints("  \033[32m上站次數：\033[37m%-35d\033[32m文章篇數：\033[37m%d (優:%d)\n", u->numlogins, u->numposts, u->goodart );
#endif
  ...
  int adm;
{
  ...
  i++;
  sprintf(buf, "性別 (0)男性 (1)女性：[%d] ", u->sex);
  if (vget(i, 0, buf, buf, 3, DOECHO))
    x.sex = (*buf - '0') & 3;
  ...
  if (adm)
  {
    ...
    sprintf(buf, "%d", u->goodart);
    vget(++i, 0, "優文數目：", buf, 10, GCARRY);
    if ((num = atoi(buf)) >= 0)
      x.goodart = num;
#ifdef BAD_POST      
    sprintf(buf, "%d", u->badart);
    vget(++i, 0, "劣文數目：", buf, 10, GCARRY);
    if ((num = atoi(buf)) >= 0)
      x.badart = num;
#endif
