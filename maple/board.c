  ...
  usint ulevel;
{
  ...
  */
  ...
  /* cache.080308: 站長擁有亦擁有板主權限 */
  else if (ulevel & PERM_SYSOP)
    bits = BRD_L_BIT | BRD_R_BIT | BRD_W_BIT | BRD_X_BIT | BRD_M_BIT;
	...
	int is_in;
{
	...
	if (brd_bits[bno] & BRD_M_BIT)
		return;

	brd = bshm->bcache + bno;

	brd_fpath(fpath, brd->brdname, "usies");
	sprintf(buf, "%s %-14s %s\n", Now(), cuser.userid, (is_in) ? "Enter" : "Leave");
	f_cat(fpath, buf);
}
  ...
  XO *xo;
{
  ...
  do
  {
	...
	continue;
      ...
      if (class_hot)
      {
        if (bshm->mantime[chn] < 5) /* 只列出人氣超過 10 的看板 */
          continue;
        bnum++;
      }
    }
  ...
  int num, bno, brdpost;
{
  ...
  /* 處理 人氣 */ /* cache.20080314: 仿ptt變色*/
  bno = bshm->mantime[bno];
  if (bno > 149)
    str3 = "\033[1;35m爆\033[m";
  else if (bno > 99)
    str3 = "\033[1;36m爆\033[m";
  else if (bno > 49)
    str3 = "\033[1;32m爆\033[m";
  else if (bno > 39)
    str3 = "\033[1;34m爆\033[m";
  else if (bno > 29)
    str3 = "\033[1;33m爆\033[m";
  else if (bno > 19)
    str3 = "\033[1;31m爆\033[m";
  else if (bno > 9)
    str3 = "\033[1;37m爆\033[m";
  else if (bno > 0)
    sprintf(str3 = buf, "%2d", bno);
  else
    str3 = "  ";

  prints("%6d%c%s%-13s\033[%sm%-5s\033[m%s ",
  ...
  XO *xo;
{
  ...
  do
  {
    ...
    prints("%6d%c  %-13.13s\033[%sm%-5.5s\033[m%s\n",
	  ...
	  str, class_color(class), str + BNLEN + 1, str + BNLEN + 1 + BCLEN + 1);
      }
      chp++;
    }
    else
