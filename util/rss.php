 ...
 ****************************************************
  Maple-itoc RSS 訂閱器 patch   
 ****************************************************
 修訂: cache.bbs   <AT> bbs.ee.ncku.edu.tw 
       renn999.bbs <AT> bbs.ccns.ncku.edu.tw
 
 此為一修訂後的衍生版本(from xdbbs to Maple-itoc)

 更新: 2009.10.15
 環境: PHP 5.2.9 with Suhosin-Patch 0.9.7
 ****************************************************
 
 
 ****************************************************
 cache.twbbs.org 的短網址產生器由 NCKU CCNS 協助維護 
 ****************************************************
 Phurl - PHP URL Shortening Script
 Powered by (c)2009 example.com

 修訂: cache.bbs    <AT> bbs.ee.ncku.edu.tw 
       rennidit.bbs <AT> bbs.ccns.ncku.edu.tw (API)      
 ****************************************************/


/* 以下設定請自行更改 */

define(SITEHOST,'nckuee.twbbs.org');
// 下載失敗時嘗試以 Proxy 下載
// 主要解決 domain 含底線"_"這個非法字元時
// 部分 OS 拒絕解析的問題
define(ALT_Proxy,'tcp://proxy.ncku.edu.tw:3128'); 
define(FOOTER,
"
--
\x1B[1;32m□ Origin: \x1B[33m風之谷 \x1B[37m bbs.ee.ncku.edu.tw \x1B[31m□ From: \x1B[37mRSS/Atom feed\x1B[m\n");
define(RSSPATH,'/home/bbs/etc/rss/');
define(RSS_AUTHOR,"RSS訂閱\器");


/****************************************************
  目前預設可選用的短網址產生器API有三個:
  您亦可架設自己的取代            
                                        
  如果您位於南台灣或者TAnet建議使用 
  http://cache.twbbs.org/api-create.php?url= 

  如果您位於北台灣建議使用 
  http://loli.tw/apiadd.php?url=

  若您使用以上的API, 請禮貌性的通知管理員 
  並限制您站台抓取 RSS 的次數低於每小時一次
  過多的短網址需要產生會導致系統無法負荷

  loli.tw 可能會有 downtime, 
  tinyurl 的 API 也是一個選擇 
  http://tinyurl.com/api-create.php?url=

  但由於連線到國外網路速度的關係, 處理會較為緩慢  
 ****************************************************/

define(SHORTURL_API, 'http://cache.twbbs.org/api-create.php?url=');
//define(SHORTURL_API, 'http://loli.tw/apiadd.php?url=');
//define(SHORTURL_API, 'http://tinyurl.com/api-create.php?url=');

/*------ 以上為設定 ------*/

include ('uao/ucs2big5uao.php');
define(FLAG_OUTGO,      1);
...
//  } rssbrdlist_t; // 200 bytes
  ...
  $channel=str_replace(chr(10),' ',ucs2big5(
  ...
  for($i = $rssobj->length - 1;$i>=0;--$i) // 從最舊的 RRS(通常在下面) 往新的掃
  {
      ...
      $title = ($title->length)?
	  ucs2big5(html_entity_decode($title->item(0)->nodeValue,ENT_QUOTES,'UTF-8')):'';
      ...
      $copy=&$description; 
      unset($description);
      $description='標題: '.substr($title,0,71).chr(10).
                   '時間: '.strftime('%D %a %X').chr(10).
		   chr(10);
      if($pubDate)
	  $description.="\x1B[1;30m發佈時間: \x1B[m".strftime('%D %a %X',$pubDate)."    ";
      if($upDate)
	  $description.="\x1B[1;30m修改時間: \x1B[m".strftime('%D %a %X',$upDate)."";
      $link=$entry->getElementsByTagName('link');
      if(strlen($title)>70)
	  $description.=chr(10)."\x1B[1;30m完整標題: \x1B[m".graceful_cut($title,69,1,"\x1B[m\n          ")."";
      if($link->length){
	  if($IsATOM)
	      $link = $link->item(0)->getAttribute('href');
	  else
	      $link = $link->item(0)->nodeValue;
	  $description.=chr(10).shorturl("\x1B[1;30m原文鏈結: \x1B[m",ucs2big5($link),true)."";
      }
      $description.=chr(10).
		    '───────────────────────────────────────'.chr(10);
      $now=0;
	  ...
	  if($IsATOM){
	      $description.=ucs2big5(
		  $author->getElementsByTagName('name')->item(0)->nodeValue).chr(27).'[m'.chr(10);
	      if( $author->getElementsByTagName('email')->length )
		  $description.=chr(27).'[1m'.'      '.ucs2big5($author->getElementsByTagName('email')->item(0)->nodeValue).chr(27).'[m'.chr(10);
	      ...
	      $description.=ucs2big5($author->nodeValue).chr(10);
      }


      $description.="\x1B[1;30m".shorturl("來源: \x1B[m",$url)."\x1B[m".chr(10).
	            "\x1B[1;30m類型: \x1B[m".(($IsATOM)?'Atom Feed':'RSS Feed')."\x1B[m".chr(10).
		    FOOTER;
...
/* 把 HTML/純文字 處理搬過來 */
  function format(&$str,$knownHTML=0){
	  ...
	  $str = str_ireplace(array('&nbsp;'),array(' '),$str);
          vlog(' -> 空白處理 Done ');
	  $str=ucs2big5($str); // 拖到現在才做，應該會快一點吧 Orz
          // 處理圖片
	  $str=preg_replace_callback('/\n?<img.*?src="(.*?)".*?>\n?/i','img_callback',$str);
      //TODO: 注意表示有可能是 <img src'URL'>
           vlog(' -> 圖片處理 Done ');     
         // 處理鏈結
